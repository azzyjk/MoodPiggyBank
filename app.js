    ...
    mysql.query('select * from member_info where id = ?',id,function(err,result){
        if(err) console.log(err);
            ...
            if(result[0] == undefined){
                mysql.query('insert into member_info (id,goal_amount,goal_reason) values (?,?,?)'
                    ,[id,goal_amount,goal_reason]
                    ,function(err,result){
                        if(err) console.log(err);
                    });
                mysql.query('insert into feeling (id) values (?)'
                    ,id
                    ,function(err,result){
                        if(err) console.log(err);
               });
            ...
            //exist member
                ...
                mysql.query('update member_info set goal_amount = ?, goal_reason = ? where id = ?'
                ,[goal_amount,goal_reason,id]
                ,function(err,result){
                    if(err) console.log(err);
                    else{
                        sendResult = {
...
app.post('/bankAddress',function(req,res){
    ...
    if(bank == '카카오뱅크') bank_encoded = process.env.kakao;
        ...
        if(err) console.log(err);
            ...
            //new member
            if(result[0] == undefined){
                mysql.query('insert into member_info (id,bank,account,amount,bank_active,send) values (?,?,?,?,1,?)'
                    ,[id,bank_encoded,account,amount,send]
                    ,function(err,result){
                    if(err) console.log(err);
                });
            ...
            //exist member
                ...
                send = 'supertoss://send?amount=' +amount+ '&bank=' + bank_encoded + '&accountNo=' + account + '&origin=qr';
                mysql.query('update member_info set bank = ?, account = ?, amount = ?, bank_active = 1, send = ?  where id = ?'
                ,[bank_encoded,account,amount,send,id]
                ,function(err,result){
                    if(err) console.log(err);
                })
            }
...
app.post('/getDairy',function(req,res){
    ...
    mysql.query('select * from member_info where id = ?',id,function(err,result){
        if(err) console.log(err);
            ...
            //new member
            if(result[0] == undefined){
               mysql.query('insert into member_info (id) values (?)'
               ,id
               ,function(err,result){
                    if(err) console.log(err);
                    else{
                    
                        mysql.query('insert into feeling (id,last_dairy) values (?,?)'
                        ,[id,text]
                        ,function(err,result){
                        if(err) console.log(err);
                        else{
                                    ...
                                    //console.log(response.data);
                                        ...
                                        kakaoData = {
                                            ...
                                            imgUrl : '218.144.108.84/' + id + '.png'
                                                ...
                                                mysql.query(sql,[Number(kakaoData.emotion),id]
                                                ,function(err,result){
                                                    if(err) console.log(err);
                                                });
            ...
            //exist member
                ...
                mysql.query('update feeling set last_dairy =  ? where id = ?'
                ,[text,id]
                ,function(err,result){
                    if(err) console.log(err);
                    else{
                                    ...
                                    //console.log(response.data);
                                        ...
                                        function(err,result){
                                            if(err) console.log(err);
                                                ...
                                                if(day == '7'){
                                                mysql.query('update feeling set print = 1 where id = ?'
                                                ,id
                                                ,function(err,result){
                                                    if(err) console.log(err);
                                                    else{
                                                       PythonShell.run('graph.py',pythonOptions,function(err,result){
                                                            if(err) console.log(err);
                                                            else{
                                                                mysql.query(sql,[Number(tmp)+Number(kakaoData.emotion),id]
                                                                ,function(err,result){
                                                                if(err) console.log(err);
                                                                 else{
                                                                    kakaoData = {
                                                                     ...
                                                                     console.log(kakaoData.emotion);
                                                                     console.log(kakaoData.sendUrl);
