...
class Network:
        ...
        for file in csv_data:
            ...
            for line in file:
                if line[1] == '' or line[2] == '':
                    ...
                    result_data[i + 1].append(int(line[2])) # Y
    ...
    def get_embedding_tfTokenizer(self, train_X, train_Y, test_X, test_Y):
        ...
        # saving
        with open('checkpoint/ML2/tokenizer.pickle', 'wb') as handle:
    ...
    def get_numpy_array(selection):
        train_X = np.load("checkpoint/ML2/" + selection + "/train_X.npy", allow_pickle=True)
        train_Y = np.load("checkpoint/ML2/" + selection + "/train_Y.npy", allow_pickle=True)
        test_X = np.load("checkpoint/ML2/" + selection + "/test_X.npy", allow_pickle=True)
        test_Y = np.load("checkpoint/ML2/" + selection + "/test_Y.npy", allow_pickle=True)
        return train_X, train_Y, test_X, test_Y

    @staticmethod
    def save_numpy_array(selection, train_X, train_Y, test_X, test_Y):
        np.save("checkpoint/ML2/" + selection + "/train_X.npy", train_X)
        np.save("checkpoint/ML2/" + selection + "/train_Y.npy", train_Y)
        np.save("checkpoint/ML2/" + selection + "/test_X.npy", test_X)
        np.save("checkpoint/ML2/" + selection + "/test_Y.npy", test_Y)
    ...
    def test(self):
        self.model.load_weights("checkpoint/ML2/variables")
        self.model.evaluate(self.test_X, self.test_Y, batch_size=60, callbacks=[self.model_checkpoint])

    def test_ready(self):
        self.model.load_weights("checkpoint/ML2/variables")
        with open('checkpoint/ML2/tokenizer.pickle', 'rb') as handle:
            self.tokenizer = pickle.load(handle)
        print("Load Model2 complete")
